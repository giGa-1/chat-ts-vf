    ...
    :
      friendsWithLastMessage.map((friend)=> (
        <div key={friend.id} className="relative bg-zinc-50 border border-zinc-200 p-3 rounded-md">
            ...
            <Link href={`/dashboasrd/chat/${chatHrefConstructor(
              session.user.id,
              friend.id
            )}`} className='relative sm:flex'>
              <div className="mb-4 flex-shrink-0 sm:mb-0 sm:mr-4 flex align-center justify-center">
                <div className="relative h-6 w-6">
                  <Image width={35} height={35} src={friend.image} alt={`${friend.name} profile pic`} className='rounded-full ' referrerPolicy='no-referrer'></Image>
                </div>
              </div>
